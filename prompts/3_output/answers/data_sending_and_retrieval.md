...
**Q: L1 What does each HTTP method do (GET, POST, PUT, PATCH, DELETE)?**
...
**Q: L1 When should you use each HTTP method?**
...
**Q: L1 Why is POST not safe and what are the practical consequences?**
...
**Q: L1 What is idempotency, which HTTP methods have it, and why does it matter for API design?**
...
**Q: L1 What's the difference between TCP and UDP, and what guarantees does TCP provide?**
...
**Q: L1 When should you use UDP over TCP?**
...
**Q: L1 Where is HTTP in the OSI model and what transport protocol does it use?**
...
**Q: L1 What is DNS's purpose and when does it run in an HTTP request?**
...
**Q: L1 What is serialization and why is it needed for network data?**
...
**Q: L1 Compare binary serialization (Protobuf, MessagePack) vs. text-based formats (JSON, XML), including trade-offs.**
...
**Q: L2 What's the high-level sequence from URL entry to first render?**
> DNS resolution → TCP connection → TLS handshake (if HTTPS) → HTTP request → server response → browser parsing and rendering.

---

**Q: L2 What happens in detail during each step of the request-to-render flow?**
...
**Q: L2 What goes in HTTP headers vs. the body? Give examples.**
...
**Q: L2 How do persistent connections work, and why were they introduced?**
...
**Q: L2 What's the difference between 401 and 403 status codes, and how should clients respond to each?**
...
**Q: L2 When should you use 307 vs 301 redirects, and what's the risk of 301 in APIs?**
...
**Q: L2 How should you handle 503 errors and when should you give up retrying?**
...
**Q: L2 How do you send a GET with `HttpClient` and read the response as a string?**
...
**Q: L2 What does `EnsureSuccessStatusCode()` do and when should you prefer manual status checking?**
...
**Q: L3 How do you POST JSON with `HttpClient` and `JsonContent`?**
...
**Q: L3 How do you send multipart/form-data (file uploads) with `HttpClient`?**
...
**Q: L3 When should you choose XML over JSON for payloads?**
...
**Q: L3 What .NET serialization classes are available for XML?**
...
**Q: L3 Why should you avoid creating `HttpClient` per request and what's the recommended pattern?**
...
**Q: L3 How do you attach a `CancellationToken` to `HttpClient`?**
...
**Q: L3 How do you distinguish user cancellation from timeout?**
...
**Q: L3 Differentiate `HttpClient.Timeout` vs `CancellationTokenSource` timeout and which takes precedence?**
...
**Q: L3 How do you connect with `TcpClient`, send a message, and read the response?**
...
**Q: L3 When should you use `UdpClient` over `TcpClient`? give real-world examples?**
...
**Q: L3 What is `ICredentials`? how does `NetworkCredential` relate to it?**
...
**Q: L3 How do Basic and NTLM authentication differ when using `NetworkCredential`?**
...
**Q: L4 Why does per-request `HttpClient` cause `SocketException`? how do you fix it?**
...
**Q: L4 Why does singleton `HttpClient` hold stale DNS entries? what's the fix?**
...
**Q: L4 What happens when JSON has unknown fields? how do you control the behavior?**
...
**Q: L4 How can mixing UTC and local time in serialization cause bugs?**
...
**Q: L4 How do you prevent UTC/local time bugs in serialization?**
...
**Q: L4 Why do circular references break JSON serialization?**
...
**Q: L4 How do you handle circular references in JSON serialization?**
...
**Q: L4 What goes wrong when retrying POST without idempotency and how do you make it safe?**
...
**Q: L4 What is a retry storm and how does jitter in backoff prevent it?**
...
**Q: L4 What's the risk of disabling SSL certificate validation and how does it expose your application?**
...
**Q: L5 Explain the `DelegatingHandler` pipeline and how requests travel through it.**
...
**Q: L5 Implement a `DelegatingHandler` that adds Bearer tokens and refreshes on 401.**
...
**Q: L5 How do you implement response caching in `HttpMessageHandler` and what headers should you respect?**
...
**Q: L5 How does `SocketsHttpHandler` pool connections and why is `PooledConnectionLifetime` important for DNS?**
...
**Q: L5 Differentiate `IdleTimeout` vs `Lifetime` and how do you tune for high-throughput services?**
...
**Q: L5 What is the TCP three-way handshake and when does `ConnectAsync` return?**
...
**Q: L5 What is `TIME_WAIT` in TCP and why does it cause port exhaustion?**
...
**Q: L6 Compare UDP vs WebSockets for real-time multiplayer communication.**
...
**Q: L6 Why is long-polling unsuitable for real-time multiplayer?**
...
**Q: L6 Compare HTTP calls vs. message queues for microservices communication.**
...
**Q: L6 When should you use HTTP calls vs. message queues?**
...
**Q: L6 What are the three main patterns for using `HttpClient` and when should you use each?**
...
**Q: L6 What makes typed clients ergonomic and how do they manage handler lifetime?**
...
**Q: L6 What's the risk when parallelizing many HTTP calls and how do you cap concurrency?**
...
**Q: L6 How do you implement capped concurrency for parallel HTTP calls?**
...
**Q: L6 Should you switch from JSON to MessagePack or Protobuf, and what should you consider?**
...
**Q: L7 How does HTTP/2 multiplexing improve over HTTP/1.1?**
...
**Q: L7 What limitation does HTTP/2 have and how does HTTP/3 improve it?**
...
**Q: L7 How do you enable HTTP/2 or HTTP/3 in `HttpClient` and what server configuration is needed?**
...
**Q: L7 How do you implement idempotency keys for financial POSTs, what should the server store, and how long?**
...
**Q: L7 What is the most important principle when implementing idempotency key handling?**
