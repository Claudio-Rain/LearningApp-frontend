...
Your task is to generate a comprehensive, progressive set of interview questions for the topic of [TOPIC].
...
- You are also free to add any area, concept, or question that a well-rounded understanding of [TOPIC] requires, even if not explicitly listed.
...
Save the generated question file inside a folder named after the topic (e.g., `questions/javascript/`, `questions/system-design/`). The filename should reflect the topic (e.g., `closures.md`, `react-hooks.md`). If the folder does not exist, create it.
